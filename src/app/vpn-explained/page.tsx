  ...
  return (
    <section className="">
      ...
      <section className="px-3">
        ...
        <p className="text-lg">
          A <b>Virtual Private Network (VPN)</b> is a type of software that runs in the background while you’re using the Internet. It establishes an encrypted connection between your device and a remote server, hiding your physical location from the websites and applications you visit. This allows you to bypass geo-restrictions and other types of internet censorship, and it also prevents third parties from seeing your browsing activity or intercepting your data on hostile networks.
        </p>

        {/* iframe */}
        <div className="bg-red-500 aspect-video">
          <iframe src="https://www.youtube-nocookie.com/embed/cAKsN4LgrDY?autoplay=1&enablejsapi=1&modestbranding=1&allowFullScreen&cc_load_policy=1&cc_lang_pref=en" allowFullScreen className="w-full h-full"></iframe>
        </div>

        <div className="my-5 bg-blue-100 p-2">
          <p className="text-xl font-semibold pb-2">Summary: What Is a VPN and Why Do You Need One?</p>
          <p className="text-xl">Use the Q&A sections below for a quick summary of what VPNs are, what they do, and what their limitations are.</p>

          <Accordion>
            {
              content1.map((itm, idx) => (
                <AccordionItem key={idx} aria-label={itm.heading} title={itm.heading}>
                  <div dangerouslySetInnerHTML={{ __html: itm.content }} className="customAccr" />
                </AccordionItem>
        ...
        <p className="my-3">In the remainder of this article, we’ll take an in-depth look at <b>everything you need to know about VPN technology and VPN services.</b></p>
        <p className="my-3">If you have a specific question about VPNs, feel free to email us or visit one of the following articles for a detailed answer:</p>
        <p className="font-semibold my-3">Have a specific question about VPNs? You can find detailed answers in the articles below:</p>
        ...
        <ul className="flex flex-wrap pl-5 list-disc list-inside">
          ...
          <li className="w-1/2 md:w-full p-2">What Are VPN Logging Policies?</li>
        </ul>

        <p className="text-3xl pt-5 font-bold">Types of VPN Connection</p>
        <p className="py-3 text-xl">There are <a href="#">different types of VPN</a> depending on the type of connection a user requires.</p>
        <p className="py-3 text-xl">Personal VPN services are designed for normal internet users who want to improve their privacy, security, and freedom online. For this reason, they are sometimes also called <b>consumer VPNs.</b></p>
        <p className="py-3 text-xl">By contrast, remote access VPNs, mobile VPNs, and site-to-site VPNs are all types of <b>business VPN.</b> They are designed for remote employees to gain secure access to internal resources.</p>
        <p className="py-3 text-xl">Here is a quick overview of each type of VPN service and how they differ from one another:</p>
        <p className="text-3xl pt-5 font-bold">Personal VPN Services</p>

        <div className="relative bg-red-800 h-60">
          <Image src="https://www.top10vpn.com/images/2021/08/Personal-VPN-Services.jpg" fill alt='personal vpn' />
        </div>
        <p className="text-center text-sm text-gray-500">A personal VPN service hides your IP address and browsing history while you browse the internet in your free time.</p>
        <p>A personal VPN service provides individual users with encrypted access to a remote VPN server that’s owned by the VPN provider.</p>
      </section>
    </section>
  );
};
